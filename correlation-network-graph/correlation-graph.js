/**
 * Functionality which generates the correlation network graph
 */
...
function render(props) {
  ...
  // setting the width for the graph.
  var parent = document.getElementById('graph');
  var parentWidth = parent.innerWidth || parent.clientWidth || 3000;
  var parentHeight = parent.innerHeight || parent.clientHeight || 3000;
  var width = parentWidth;
  var height = parentHeight;

  // setting up linkweight and threshold. As the correlation increases the linkweight increases.
  ...
  var linkWidthScale = d3.scalePow().exponent(2).domain([0, 1]).range([0, 2]);
  ...
  /**
   * calculate degree of freedom for each node
  ...
  /**
   * jLouvian is an already implemented community detection algorithm.
   * We take in the JS file jLouvian and call the function to detect the
   * communities using the nodes and edges.
   */
  ...
  // community radius is defined in such a way the nodes in the same communities
  // are grouped closer to eachother
  //
  var defaultRadius = 2;
  ...
  function drawSliderControl(props) {
    ...
    d3.select(selector).append('input').attr('type', 'range').attr('min', 0).attr('max', 1).attr('value', 0.356).attr('step', 0.001).style('top', '604px').style('left', '340px').style('height', '36px').style('width', '760px').style('position', 'fixed').attr('id', 'slider');
